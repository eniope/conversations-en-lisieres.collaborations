---
type: index
status: en-cours
title: "Labo Penser ensemble l'oeuvre à faire"
subtitle: "Éprouver les enjeux du faire ensemble"
description: "Un espace d’essais et d’expérimentation pour s'exercer aux pratiques collectives"
tags:
  - labo
  - processus
  - penser
  - experimentation
  - seuil
navigation:
  section: "Labo"
  order: 1
visibility:
  toc: false
  backlinks: true
---
# Labo Penser ensemble l'oeuvre à faire
Ce labo est un espace d’essais. Il ne présente pas des résultats, ne transmet pas une méthode, ne cherche pas à convaincre.
Il propose des situations à éprouver pour observer ce qui se joue dans les dynamiques collectives au sens large. "L'intelligence collective pour un groupe c'est sa capacité à se formuler un futur et à y parvenir dans un contexte complexe" Jean François Noubel- L'intelligence collective, la révolution invisible-2004

## Pourquoi un Labo

Dans nos expérimentations collectives, certains phenomènes créent du trouble dont la portée n'est pas directement comprise dans l'action.
Elles résistent à la mise en mots. Elles produisent un « tiens, c’est bizarre ».
Ce labo existe pour travailler ce moment-là. Non pour le résoudre, mais pour l’habiter assez longtemps afin de voir ce qui s’y joue. Ici, on ne part pas d’une thèse, on part d’un frottement avec le réel. 
## Ce qui est proposé ici
Dans ce labo, tu trouveras : 

- des jeux sérieux
- des fragments d’écriture,
- des essais de formulation,
- des déplacements de notions,
- des formes encore instables,
- des propositions à manipuler ou à prolonger.

Chaque objet vise moins à dire quelque chose qu’à faire éprouver un passage, celui par lequel un trouble se transforme, se structure, ou parfois se défait. 
## Ce que ce Labo n’est pas

Ce Labo n’est pas :
- un espace d’explication,
- un lieu de démonstration,
- un atelier pédagogique,
- un making-of de textes finis.

Il n’y a rien à réussir ici, rien à comprendre entièrement, rien à valider.
## Comment entrer
Tu peux entrer par n’importe quel objet, lire peu ou beaucoup, t’arrêter rapidement, revenir plus tard. Tu peux simplement observer ou bien t’engager davantage, tester une idée, reprendre une formulation, laisser une question ouverte. 
Ce qui compte, ce n’est pas ce que tu emportes, mais ce que cela te permet d’éprouver.
## Une attention partagée
Ce labo part d’un postulat simple : le pouvoir de penser n’a pas disparu, mais ses conditions sont souvent fragilisées.
Ici, nous faisons le pari inverse : que des conditions modestes, situées, non spectaculaires peuvent rouvrir des prises.
Ce labo n’extrait rien, n’optimise rien, il propose.


